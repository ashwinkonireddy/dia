...
class php_entry_saver_base {
    ...
    function __construct($formId,$formString,$captcha,$referenceId="")
    {
        ...
        $this->ReferenceId=substr(md5(uniqid(time()+mt_rand())), -6);
        $this->GetFormOptions();
    }
...
//        if($this->FormOptions["SendNotificationEmail"]=="y")
            $this->SendFormEmail($this->FormOptions["Emails"][0],$this->FormEntryData,$this->ElementOptions,false,$this->ReferenceId);
    ...
    public static function SendFormEmail($formOptions,$entryData,$elementOptions,$useTestData,$refCode)
    {

//        var_dump($entryData['rnField2']['value']);
//        var_dump($formOptions,$entryData,$elementOptions,$useTestData);die;
...
//        $EmailSubject=$formOptions["EmailSubject"];
        ...
        $EmailText=$formOptions["EmailText"];
        $FromName= "Dentistin Awards";
        $FromEmail="info@dentistinawards.org.uk";
        $ToEmail=$entryData['rnField2']['value'];
        $EmailSubject="Dentistin Awards Registration";
